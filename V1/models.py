...
class Lattice_Transformer_SeqLabel(nn.Module):
    def __init__(self, lattice_embed, bigram_embed, hidden_size, label_size,
        ...
        ## change the logic of setting char_input_size/lex_input_size HERE, can not run NER task
        ...
        if self.use_bert:
            ...
            self.char_input_size = self.bert_embedding._embed_size
            self.lex_input_size = self.bert_embedding._embed_size


        # # control model whether to use
        # # TODO: build self.char_vocab
        ...
        #         bert_embedding,
        #         self.char_input_size,
        #         self.char_input_size // 2,  # Half-size to leave room for BLSTM
        ...
        print('CEHCK label_size IN model: ', self.label_size)       
        self.crf = get_crf_zero_init(self.label_size)
        self.loss_func = nn.CrossEntropyLoss(ignore_index=-100)


    def forward(self, lattice, bigrams, seq_len, lex_num, pos_s, pos_e, target, chars_target=None):
        ...
        # print('CEHCK batch_size HERE: ', batch_size)
        max_seq_len_and_lex_num = lattice.size(1)
        # print('CEHCK max_seq_len_and_lex_num HERE: ', max_seq_len_and_lex_num)

        # print('CEHCK seq_len HERE: ', seq_len)
        # print('CEHCK lex_num HERE: ', lex_num)

        # print('CEHCK pos_s HERE: ', pos_s.size())
        # print('CEHCK pos_e HERE: ', pos_e.size())

        max_seq_len = bigrams.size(1)

        # raw_embed = self.lattice_embed(lattice)

        # to deal with character-level embedding, that is raw_embed_char
        # if self.use_bigram:

        #     bigrams_embed = self.bigram_embed(bigrams)
        #     bigrams_embed = torch.cat([bigrams_embed,
        #                                torch.zeros(size=[batch_size, 
        #                                                  max_seq_len_and_lex_num - max_seq_len, 
        #                                                  self.bigram_size]).to(bigrams_embed)
        #                                                 ], 
        #                                 dim=1)
        #     # concat in raw_embed (that is lattice ) shape!!!!!
        ...
        if self.use_bert:
            ...
            raw_bert_embed = self.bert_embedding(lattice)


            # mask = seq_len_to_mask(seq_len).bool()
            # char_for_bert = lattice.masked_fill((~mask), self.vocabs['lattice'].padding_idx)

            # bert_embed_char = self.bert_embedding(char_for_bert)
            # bert_embed_char = torch.cat([bert_embed_char, torch.zeros(size=[batch_size, bert_pad_length, bert_embed_char.size(-1)],
            #                              device = bert_embed_char.device, requires_grad=False)], dim=-2)

            # if self.embed_dropout_pos == '0':
            #     bert_embed_char = self.embed_dropout(bert_embed_char)
            
            embed_char = self.char_proj(raw_bert_embed)

            char_mask = seq_len_to_mask(seq_len, max_len=max_seq_len_and_lex_num).bool()
            embed_char.masked_fill_(~(char_mask.unsqueeze(-1)), 0)

            # bert_embed_lex = self.bert_embedding(lattice)

            embed_lex = self.lex_proj(raw_bert_embed)
            if self.mode['debug']:
        ...
        # merge the embed_char and embed_lex HERE!!!!!
        # [TODO] to check if the shape of these two embeds are same, if so '+'==point addition
        # if not self.use_lexicon and not self.use_rule:
        ...
        # print('CEHCK embedding IN Lattice_Transformer_SeqLabel: ', embedding.size())
        ...
        # Truncate directly using max_seq_len
        encoded = encoded[:, :max_seq_len, :]
        pred = self.output(encoded)
        # Then use 2-D mask from 1-D seq_len to precisely pick out character output
        mask = seq_len_to_mask(seq_len).bool()

        if self.training:
            # print('CEHCK pred IN model: ', pred.size())
            # print('CEHCK target IN model: ', target)
            # print('CEHCK mask IN mask: ', mask)

            loss = self.crf(pred, target, mask).mean(dim=0)

            if self.self_supervised: # default == False
...
class BERT_SeqLabel(nn.Module):
    def __init__(self, bert_embedding, label_size, vocabs,after_bert):
        ...
        if self.after_bert == 'lstm':
            self.lstm = LSTM(bert_embedding._embed_size, bert_embedding._embed_size//2, bidirectional=True)
    ...
    def forward(self, lattice, bigrams, seq_len, lex_num, pos_s, pos_e,
        ...
        if self.after_bert == 'lstm':
            encoded, _ = self.lstm(encoded, seq_len.cpu())
